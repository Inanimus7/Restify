$(document).ready(function() {
    ...
    $("#check-in-date, #check-out-date").datepicker({
        ...
        beforeShow: function(input, inst) {
            ...
            return true; // Ensure the datepicker opens normally
        },
...
$(document).ready(function() {
    $('#search-btn').click(function(event) {
        ...
        if (!checkInDate || !checkOutDate || checkInDate >= checkOutDate) {
            ...
            $('#date-error-message').show(); // Display the error message
            $('#date-error-message').addClass('shake'); // Add the shake effect to the form

            // Remove the shake class after animation ends
            setTimeout(function() {
                $('#date-error-message').removeClass('shake');
            }, 500); // 500ms is the duration of the shake
        }
    });
});